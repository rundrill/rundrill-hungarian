...
Your personal **Hungarian coach** inside your AI agent — short production-first drills, an honest
...
The coach explains in your native language and moves toward Hungarian as your level rises
(comprehensible input, i+1), and drills what Hungarian actually turns on — vowel harmony, agglutination with ~18 case suffixes, definite vs indefinite conjugation, possessive suffixes, and focus word order.

## One plugin, three hosts

The coaching skill (`skills/hungarian-coach/SKILL.md`) and `.mcp.json` are shared; each host reads its
...
- **Claude Code / Desktop** — via the RunDrill languages marketplace:
  ...
  /plugin install rundrill-hungarian@rundrill-lang
  ```
  Then run `/hungarian-coach`.
- **OpenAI Codex** — add the RunDrill languages catalog, then install `rundrill-hungarian`:
  ```
  codex plugin marketplace add rundrill/rundrill-lang
  ```
- **Google Antigravity** — drop this folder into `~/.gemini/config/plugins/rundrill-hungarian/` (global)
  or `<workspace>/.agents/plugins/rundrill-hungarian/` (workspace-scoped).
...
Required attribution when sharing: *“RunDrill Hungarian coach — © RunDrill, CC BY-NC-ND 4.0,
https://rundrill.com”.*
